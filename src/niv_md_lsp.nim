...
type
  ...
  MdSectionArgs = object
    textPtr: ptr UncheckedArray[char]
    textLen: int
    startPos: int
    endPos: int
    startLine: int
    initState: MdTokenizerState
    chanIdx: int

var mdSectionChannels: array[MaxTokenThreads, Channel[seq[MdToken]]]
var mdSectionThreads: array[MaxTokenThreads, Thread[MdSectionArgs]]
...
proc preScanMdState(text: string, startPos, endPos: int, startLine: int, initState: MdTokenizerState): MdTokenizerState =
  ## Lightweight pre-scan tracking only cross-line state variables.
  ## Operates on raw text bytes (not split into lines) for speed.
  result = initState
  var pos = startPos
  ...
  while pos < endPos:
    ...
    # --- Multi-line HTML comment continuation ---
    if result.inHtmlComment:
      if c == '-' and pos + 2 < endPos and text[pos+1] == '-' and text[pos+2] == '>':
        result.inHtmlComment = false
        pos += 3
      else:
        inc pos
      continue

    # --- Frontmatter: first line must be --- ---
    if lineNum == startLine and lineNum == 0 and not result.frontmatterDone and not result.inFrontmatter:
      # Check if line starts with ---
      var lp = pos
      while lp < endPos and text[lp] in {' ', '\t'}: inc lp
      if lp + 2 < endPos and text[lp] == '-' and text[lp+1] == '-' and text[lp+2] == '-':
        var ep = lp + 3
        var allDash = true
        while ep < endPos and text[ep] != '\n':
          if text[ep] notin {'-', ' ', '\t'}:
            allDash = false
            break
          inc ep
    ...
    if result.inFrontmatter:
      ...
      if lp + 2 < endPos and text[lp] == '-' and text[lp+1] == '-' and text[lp+2] == '-':
        ...
        while ep < endPos and text[ep] != '\n':
          ...
          inc ep
        if onlyWs:
          result.inFrontmatter = false
          result.frontmatterDone = true
          pos = ep
          continue
      if lp + 2 < endPos and text[lp] == '.' and text[lp+1] == '.' and text[lp+2] == '.':
        var ep = lp + 3
        ...
        if onlyWs:
          ...
          pos = ep
          continue
      # Skip rest of frontmatter line
      while pos < endPos and text[pos] != '\n': inc pos
      continue

    if not result.inFrontmatter and not result.frontmatterDone:
      result.frontmatterDone = true

    # --- Fenced code blocks ---
    if result.inFencedCode:
      # Check for closing fence at start of line
      var indent = 0
      var lp = pos
      while lp < endPos and text[lp] == ' ':
        inc indent
        inc lp
      if lp < endPos and text[lp] == result.fenceChar:
        var count = 0
        while lp < endPos and text[lp] == result.fenceChar:
          inc count
          inc lp
        var onlyWs = true
        while lp < endPos and text[lp] != '\n':
          if text[lp] notin {' ', '\t'}:
            ...
            break
          inc lp
        if count >= result.fenceLen and onlyWs:
          result.inFencedCode = false
          pos = lp
          continue
      # Skip rest of fenced code line
      while pos < endPos and text[pos] != '\n': inc pos
      continue

    # Check for opening fence
    block checkOpenFence:
      var indent = 0
      var lp = pos
      while lp < endPos and text[lp] == ' ' and indent < 4:
        inc indent
        inc lp
      if lp < endPos and text[lp] in {'`', '~'}:
        let fc = text[lp]
        var count = 0
        while lp < endPos and text[lp] == fc:
          inc count
          inc lp
        if count >= 3:
          result.inFencedCode = true
          result.fenceChar = fc
          result.fenceLen = count
          result.fenceIndent = indent
          # Skip rest of line (language info etc)
          while lp < endPos and text[lp] != '\n': inc lp
          pos = lp
          continue

    # Check for HTML comment start in inline content
    if c == '<' and pos + 3 < endPos and text[pos+1] == '!' and text[pos+2] == '-' and text[pos+3] == '-':
      pos += 4
      # Look for close on same line
      var foundClose = false
      while pos < endPos and text[pos] != '\n':
        if text[pos] == '-' and pos + 2 < endPos and text[pos+1] == '-' and text[pos+2] == '>':
          pos += 3
          foundClose = true
          break
        inc pos
      if not foundClose:
        # Reached newline without closing -> multi-line comment
        result.inHtmlComment = true
      continue

    # Skip rest of line
    while pos < endPos and text[pos] != '\n': inc pos

proc mdSectionWorker(args: MdSectionArgs) {.thread.} =
  ## Tokenize a section of Markdown text with given initial state and startLine.
  ...
  var inFencedCode = args.initState.inFencedCode
  var fenceChar = args.initState.fenceChar
  var fenceLen = args.initState.fenceLen
  var fenceIndent = args.initState.fenceIndent
  var inHtmlComment = args.initState.inHtmlComment
  var inFrontmatter = args.initState.inFrontmatter
  var frontmatterDone = args.initState.frontmatterDone

  for localLineNum in 0..<lines.len:
    let line = lines[localLineNum]
    let lineNum = args.startLine + localLineNum
  ...
  mdSectionChannels[args.chanIdx].send(tokens)

proc tokenizeMdParallel(text: string): seq[MdToken] =
  ...
  var states: seq[MdTokenizerState] = @[MdTokenizerState()]
  for t in 0..<threadCount - 1:
    ...
    states.add(preScanMdState(text, startPos, endPos, sLine, states[t]))
  ...
  for t in 0..<threadCount:
    ...
    let startPos = lineOffsets[sLine]
    let endPos = if eLine + 1 < lineOffsets.len: lineOffsets[eLine + 1] else: text.len
    mdSectionChannels[t].open()
    createThread(mdSectionThreads[t], mdSectionWorker, MdSectionArgs(
      textPtr: textPtr, textLen: text.len,
      startPos: startPos, endPos: endPos,
      startLine: sLine, initState: states[t], chanIdx: t
    ))

  var allTokens: seq[MdToken] = @[]
  for t in 0..<threadCount:
    joinThread(mdSectionThreads[t])
    let (hasData, sectionTokens) = mdSectionChannels[t].tryRecv()
    if hasData: allTokens.add(sectionTokens)
    mdSectionChannels[t].close()
...
proc tokenizeMdRange(text: string, startLine, endLine: int): seq[MdToken] =
  let allTokens = tokenizeMdParallel(text)
  result = @[]
    ...
    of "textDocument/semanticTokens/full":
      ...
      for doc in documents:
        if doc.uri == uri:
          text = doc.text
          break
      let tokens = tokenizeMdParallel(text)
