...
proc tokenizePythonRange(text: string, startLine, endLine: int): seq[PythonToken] =
  ...
  proc inClassScope(): bool =
    for i in countdown(scopeStack.len - 1, 0):
      if scopeStack[i].kind == skClass: return true
      if scopeStack[i].kind == skFunction: return false
    return false

  proc readStringRange(quoteChar: char, isRaw: bool): (int, int, int) =
  ...
  while pos < text.len:
    ...
    of '#':
      ...
      if sLine >= startLine and sLine <= endLine:
        result.add(PythonToken(kind: ptComment, line: sLine, col: sCol, length: length))
    ...
    of 'a'..'z', 'A'..'Z', '_':
      ...
      if c in {'f', 'F', 'r', 'R', 'b', 'B', 'u', 'U'}:
        ...
        if nextPos < text.len and text[nextPos] in {'\'', '"'}:
          ...
          for _ in 0..<prefixLen: advance()
          let quoteChar = text[pos]
          let (strLine, _, strLen) = readStringRange(quoteChar, isRaw)
          emitStringTokens(result, text, startPos, pos, sLine, sCol, line,
                           startLine, endLine)
      ...
      if not isStringPrefix:
        ...
        if word == "def" or word == "class":
          ...
          if inRange():
            if isMethod:
              result.add(PythonToken(kind: ptMethod, line: sLine, col: sCol, length: length))
            else:
          ...
          if inRange():
            result.add(PythonToken(kind: ptClass, line: sLine, col: sCol, length: length))
          scopeStack.add(ScopeEntry(kind: skClass, indent: lineIndent))
          lastKeyword = ""
        elif inFuncParams and not afterParamColon:
          if inRange():
            if (word == "self") and isFirstParam:
              result.add(PythonToken(kind: ptSelfParam, line: sLine, col: sCol, length: length))
            elif (word == "cls") and isFirstParam:
              result.add(PythonToken(kind: ptClsParam, line: sLine, col: sCol, length: length))
            else:
              result.add(PythonToken(kind: ptParameter, line: sLine, col: sCol, length: length))
          expectParam = false; isFirstParam = false
        elif inImportLine:
          if inRange():
            result.add(PythonToken(kind: ptNamespace, line: sLine, col: sCol, length: length))
        elif inFromLine:
          if inRange():
            result.add(PythonToken(kind: ptNamespace, line: sLine, col: sCol, length: length))
        elif wasDot:
    ...
    of '"', '\'':
      lastKeyword = ""; afterDot = false
      let quoteChar = c; let sLine = line; let sCol = col
      let stringStartPos = pos
      let (_, _, length) = readStringRange(quoteChar, false)
      emitStringTokens(result, text, stringStartPos, pos, sLine, sCol, line,
                       startLine, endLine)
    ...
    of '0'..'9':
      ...
      if pos < text.len and text[pos] in {'j', 'J'}:
        advance(); inc length
      if inRange():
        result.add(PythonToken(kind: ptNumber, line: sLine, col: sCol, length: length))
    ...
    of '@':
      ...
      while pos < text.len and (isIdentChar(text[pos]) or text[pos] == '.'):
        advance(); inc length
      if length > 1 and inRange():
        result.add(PythonToken(kind: ptDecorator, line: sLine, col: sCol, length: length))

    of '(':
      lastKeyword = ""; afterDot = false
      if inFuncParams: inc funcParamDepth
      elif funcParamDepth == 0 and result.len > 0 and
           result[^1].kind in {ptFunction, ptMethod}:
